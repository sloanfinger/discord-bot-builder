...
export default function IndexPage() {
    ...
    if (router.query.catchall[router.query.catchall.length - 1] !== 'create') return Throw404();

    const reducer: ((p: any[], c: any) => typeof p) = (p: any[], c: any) => {
        return c.actions ? p.concat(([{...c, actions: true}]).concat(Array.apply(null, c.actions).reduce(reducer, [] as any[]))) : p.concat([c]);
    }
    const allActions = event.actions.reduce(reducer, [] as any[]);
    const allActionKeys = allActions.map(action => action.key);

    const queries = (router.query.catchall as string[]).filter((query, index) => router.query.catchall[index - 1] === 'action');
    if (queries.filter((actionKey) => allActionKeys.indexOf(actionKey) + 1).length !== queries.length) return Throw404();
